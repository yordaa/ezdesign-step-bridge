...
Do not add that override or remove the vendored tree in this combined repository. Issue #10 remains blocked by [issue #9](https://github.com/yordaa/ezdesign-step-bridge/issues/9), which must first establish the standalone source and registry repositories. After that split, carry this baseline into the source repository, place the override in the registry, and rerun the same macOS arm64 measurements plus Windows x64 CI.
